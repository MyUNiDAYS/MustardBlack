...
namespace MustardBlack
{
	...
	{
		static readonly MimeMappingDictionaryBase mappingDictionary = new MimeMappingDictionaryClassic();
		...
		{
			readonly Dictionary<string, string> mappings = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
			static readonly char[] pathSeparatorChars = { Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar, Path.VolumeSeparatorChar };
			...
			public string GetMimeMapping(string fileName)
			{
				...
				for (int i = 0; i < fileName.Length; i++)
				{
					if (fileName[i] == '.')
					{
						...
						string mimeType;
						if (mappings.TryGetValue(fileName.Substring(i), out mimeType))
						{
							// found!
							return mimeType;
						}
		...
		sealed class MimeMappingDictionaryClassic : MimeMappingDictionaryBase
		{
			protected override void PopulateMappings()
			{
				...
				AddMapping(".woff", "font/woff");
				AddMapping(".woff2", "font/woff2");
