...
namespace MustardBlack.ViewEngines.Razor
{
	...
	public class ExpressionRewriter : CSharpSyntaxRewriter
	{
		...
		public ExpressionRewriter(SemanticModel semanticModel)
		{
			...
			this.Expressions = new List<KeyValuePair<SimpleLambdaExpressionSyntax, IdentifierNameSyntax>>();
		}
		...
		public override SyntaxNode VisitClassDeclaration(ClassDeclarationSyntax node)
		{
			...
			foreach (var kvp in this.Expressions)
			{
				var expression = kvp.Key;
				var memberName = kvp.Value.GetFirstToken();

				var expressionType = this.SemanticModel.GetTypeInfo(expression).ConvertedType;
		...
		{
			Debug.Assert(this.IsInsideClass);
			...
			//      }
			//
			var type = this.SemanticModel.GetTypeInfo(node);
			...
			if (IsValidForHoisting(parameter, node.Body))
			{
				...
				var memberName = string.Format(FieldNameTemplate, this.Expressions.Count);
				var memberAccess = PadMemberAccess(node, SyntaxFactory.IdentifierName(memberName));
				this.Expressions.Add(new KeyValuePair<SimpleLambdaExpressionSyntax, IdentifierNameSyntax>(node, memberAccess));
				return memberAccess;
			}
